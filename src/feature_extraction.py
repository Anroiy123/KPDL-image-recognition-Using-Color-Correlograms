...
def extract_all_features(
    ...
    for i, img in enumerate(images):
        ...
        if (i + 1) % 50 == 0 or (i + 1) == total:
            print(
                f"  [{method}/{color_space}] {i+1}/{total} " f"({100*(i+1)/total:.0f}%)"
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Trich xuat dac trung cho dataset profile"
    )
    parser.add_argument(
        "--dataset-profile",
...
def main(dataset_profile_key=DEFAULT_DATASET_PROFILE):
    ...
    for key, value in output_values.items():
        output_path = scoped_artifact_path(
            features_dir, dataset_profile["key"], FEATURE_OUTPUT_FILES[key]
        )
    ...
    print(
        f"  - {output_paths['correlogram_hsv_spatial'].name}: {X_corr_hsv_spatial.shape}"
    )
