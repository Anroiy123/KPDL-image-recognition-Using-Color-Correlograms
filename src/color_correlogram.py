...
def _validate_quantized_image(
    ...
    if quantized_img.dtype not in [np.int32, np.int64, np.int16, int]:
        raise TypeError(
            f"{param_name} must have integer dtype, got {quantized_img.dtype}"
        )
...
def _validate_distances(
    distances: Optional[List[int]], param_name: str = "distances"
) -> None:
    """Validate distances parameter."""
    if distances is not None:
        if not isinstance(distances, (list, tuple)):
            raise TypeError(
                f"{param_name} must be list/tuple or None, got {type(distances)}"
            )
        if len(distances) == 0:
        ...
        for d in distances:
            if not isinstance(d, (int, np.integer)) or d <= 0:
                raise ValueError(
                    f"{param_name} values must be positive integers, got {distances}"
                )


def auto_correlogram(
    quantized_img: np.ndarray, n_colors: int, distances: Optional[List[int]] = None
) -> np.ndarray:
    ...
    for d_idx, d in enumerate(distances):
        ...
        shifts = [(-d, -d), (-d, 0), (-d, d), (0, -d), (0, d), (d, -d), (d, 0), (d, d)]
...
def auto_correlogram_fast(
    ...
    for d_idx, d in enumerate(distances):
        ...
        for dy, dx in shifts:
            ...
            neighbor = quantized_img[
                y1_src + dy : y2_src + dy, x1_src + dx : x2_src + dx
            ].ravel()
            ...
            total_match += np.bincount(center[match], minlength=n_colors).astype(
                np.float64
            )
...
def _quantize_image_for_correlogram(
    img_bgr: np.ndarray,
    color_space: str = "hsv",
    h_bins: int = 8,
    s_bins: int = 3,
...
def extract_correlogram_feature(
    img_bgr: np.ndarray,
    color_space: str = "hsv",
    h_bins: int = 8,
    s_bins: int = 3,
    ...
    rgb_bins: int = 4,
    distances: Optional[List[int]] = None,
    spatial_grid: Optional[int] = None,
    include_global: bool = True,
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    print(
        f"Vector HSV: min={feat3.min():.4f}, max={feat3.max():.4f}, mean={feat3.mean():.4f}"
    )
