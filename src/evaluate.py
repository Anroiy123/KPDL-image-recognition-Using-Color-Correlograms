...
EXPERIMENTS = [
    (
        "SpatialCorrelogram_HSV_SVM",
        "correlogram_hsv_spatial",
...
def plot_accuracy_comparison(results, save_path):
    ...
    ax.set_title(
        "So sanh Accuracy tren Tap Test Doc Lap", fontsize=14, fontweight="bold"
    )
    ax.set_ylim(0, 100)
...
def plot_per_class_comparison(results_dict, class_names, save_path):
    ...
    ax.set_title(
        "So sanh Accuracy Theo Lop Tren Tap Test", fontsize=14, fontweight="bold"
    )
    ax.set_xticks(x)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Danh gia held-out test theo dataset profile"
    )
    parser.add_argument(
        "--dataset-profile",
...
def main(dataset_profile_key=DEFAULT_DATASET_PROFILE):
    ...
    for key, base_name in FEATURE_FILES.items():
        path, is_legacy = resolve_scoped_or_legacy_path(
            features_dir, dataset_profile["key"], base_name
        )
        if not path.exists():
            raise FileNotFoundError(
    ...
    feature_matrices = {
        ...
        "correlogram_hsv_spatial": np.load(
            resolved_feature_paths["correlogram_hsv_spatial"]
        ),
    ...
    for name, feature_key, model_file in EXPERIMENTS:
        print(f"\n--- Danh gia: {name} ---")
        model_path, is_legacy_model = resolve_scoped_or_legacy_path(
            models_dir, dataset_profile["key"], model_file
        )
    ...
    if all_results:
        ...
        plot_accuracy_comparison(
            all_results, results_dir / f"accuracy_comparison_{result_prefix}.png"
        )

        if (
            "SpatialCorrelogram_HSV_SVM" in predictions
            and "Histogram_HSV_SVM" in predictions
        ):
            plot_per_class_comparison(
                {
                    "Spatial Correlogram (HSV) + SVM": predictions[
                        "SpatialCorrelogram_HSV_SVM"
                    ],
    ...
    for r in all_results:
        print(
            f"{r['name']:<30} {r['accuracy']:.4f}  {r['precision']:.4f}  {r['recall']:.4f}  {r['f1_score']:.4f}"
        )

    summary_path = results_dir / scoped_artifact_name(
        dataset_profile["key"], "evaluation_summary.json"
    )
